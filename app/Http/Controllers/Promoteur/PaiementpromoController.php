...
class PaiementpromoController extends Controller
{
    public function paiements(){
        ...
        $annonce=Annonce::all();
        return view('promoteur.mains-promoteur.paiement.add',compact('annonce'));
        }
        public function paiementAdd(Request $request){
          ...
          Paiement::insert([
            ...
            return Redirect::to("promoteur/paiement")->with('success', 'le paiement est ajoutée avec succes');
    
        }
        public function paiementShow($id){
            $paiement=Paiement::find($id);
            $annonce=Annonce::all();
            return view('promoteur.mains-promoteur.paiement.edit',compact('paiement','annonce'));
        }
        public function paiementUpdate(Request $request){
            ...
            paiement::findOrFail($paiement_id)->update([
              ...
              return Redirect::to("promoteur/paiement")->with('success', 'le paiement est modifiée avec succes');
            }
            public function  deletepaiment($id){
              $paiement=Paiement::findOrfail($id);
              unlink($paiement->photo_virement);
                Paiement::findOrFail($id)->delete();
                return Redirect::to("promoteur/paiement")->with('success', 'le paiement est supprimée avec succes');
